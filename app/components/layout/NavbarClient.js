...
import LangToggle from '@/app/ui/LangToggle'
...
export default function NavbarClient() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(logo, { opacity: 0, y: -20, duration: 1, ease: 'power3.out' })
    }, navRef)
  ...
  return (
    <nav
      ref={navRef}
      className='z-5 grid grid-cols-[0.5fr_1.8fr] w-[100vw] items-center bg-[var(--bg-light)] fixed top-0 xl:h-[7rem] lg:h-[6.7rem] sm:h-[6.2rem] h-[4.2rem] xl:px-8 lg:px-6 sm:px-4 px-3'
    >
...
export const NavLink = ({ data, h }) => {
  return (
    <Link
      className={`${rubik.className} text-[1.35rem] tracking-[1.5px] hover:text-[var(--primary)] font-semibold capitalize`}
      href={h}
    >
      {data}
    </Link>
